...
print(gtfs$tidytransit$date_service_table)
```
...
calendar = tibble(date = unique(gtfs$tidytransit$date_service_table$date)) %>% 
  mutate(
...
to an existing `service_id` (and by extension `trip_id`). In addition, `gtfs$tidytransit$date_servicepattern_table` 
...
length(unique(gtfs$tidytransit$service_servicepattern$servicepattern_id)) # 7
```
...
date_servicepattern_table <- gtfs$tidytransit$date_servicepattern_table %>% left_join(calendar, by = "date")
...
```{r}
servicepattern_names = gtfs$tidytransit$date_servicepattern_table %>% 
...
```{r fig.height=4, fig.width=7}
dates = gtfs$tidytransit$date_servicepattern_table
dates$wday <- lubridate::wday(gtfs$tidytransit$date_servicepattern_table$date, label = T, abbr = T)
...
```{r fig.height=4, fig.width=7}
trips_servicepattern = left_join(select(gtfs$trips, trip_id, service_id), gtfs$tidytransit$service_pattern, by = "service_id")
trip_dates = left_join(gtfs$tidytransit$date_servicepattern_table, trips_servicepattern, by = "servicepattern_id")
