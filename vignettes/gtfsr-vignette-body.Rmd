...
map_gtfs(gtfs_obj = duke_gtfs_obj, route_ids = C1_route_id) # map route shape with stops
...
map_gtfs_routes(gtfs_obj = duke_gtfs_obj, C1_CCX_route_ids) # map route shape with stops (the default)
```

Finally, we can get visualize all of the routes that make up Duke University's Transit system using `map_gtfs_agency_network`. Duke University Transit system is made of only one agency (`duke_agency_name = "Duke Transit"`). If desired, we can also omit stops for every route in the network by using option `include_stops = FALSE` (this option is `include_stops = TRUE` by default).
...
map_gtfs_agency_network(gtfs_obj = duke_gtfs_obj, agency_name = duke_agency_name) # map all routes of agency with stops (the stops)
map_gtfs_agency_network(gtfs_obj = duke_gtfs_obj, agency_name = duke_agency_name, include_stops = FALSE) # map all routes of agency, with no stops
```
