...
The focus of this vignette is on how to use R to make graphics about where and how often transit service operates based on schedule data published in the [General Transit Feed Specification](http://gtfs.org/). 

We're going to review how to use this package to:

1) Import Transit Data (GTFS)
2) Identify Weekday Schedules of Service
3) Calculate Schedule Headways
...
Given the one-to-one relationship between service-id's and routes, we might conclude that the NYC Subway GTFS creates service_id's for each route that a trip runs on. Some GTFS feeds are simpler: a single service_id might relate to 'all vehicle trips running every weekdays'. 
