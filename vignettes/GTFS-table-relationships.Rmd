...
tidytransit prints a message regarding these tables on read. 
...
## Join Calculated Tables to GTFS Tables

Each of these can be joined back to standard GTFS tables, or to each other, with more information using either route_id or stop_id. 
...
routes_df_frequencies <- nyc$routes_df %>% 
  dplyr::inner_join(nyc$routes_frequency_df, by = "route_id") %>% 
  dplyr::select(route_long_name,
                median_headways, 
                 mean_headways, 
                 st_dev_headways, 
                 stop_count)
