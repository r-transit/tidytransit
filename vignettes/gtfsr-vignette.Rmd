...
Once we have the urls for the feeds of interest, we can download and extract all the GTFS data into a data list (aka a list of data frames) using the `get_gtfs` data. To save time, we will only download 2 feeds (`slice(8:9)`). The output is suppressed because of how verbose it is.

```{r get_gtfs, warning=FALSE, message=FALSE, echo=TRUE, eval=FALSE}
...
```
NOTE: Parsing errors and warnings while importing data can be extracted by setting the `problems` option to TRUE (problems = TRUE).
```

This output was suppressed in the last section to save space given how verbose it is. But the highlighted `NOTE` explains that *if one observes an error or warning during the import process*, one can use the `problems = TRUE` option (`FALSE` by default), which allows you to extract a problems data frame.
...
prob_list1 <- url %>% get_gtfs(problems = TRUE)
```
...
To understand the problem, let's compare one of the data frames with problems to what was imported. In this example, we select `calendar_df`.


```{r title, echo=TRUE, eval=TRUE}
data_list1[[1]]$calendar_df
prob_list1[[1]]$calendar_df
```

From inspecting the output, it appears the problems (at least for `calendar_df`) stem from the possibility that empty rows were added to the end of the original text file. Not a big deal and easily fixed. But we leave such specific fixes to the user to correct.
