...
Another reason is that the headway calculations in this package aren't robust against all of the edge cases of every last service and stops that might be listed in a GTFS. Remember, GTFS isn't built for this kind of calculation--its built to publish schedules to users. 

However, I have found that the methods in here are OK at describing transit service headways on routes and stops if you understand that GTFS data can be messy for analytical work, and may require a trick here or there in order to get to the point. 

One quick solution to the above plot is to throw out stops with headways greater than an unreasonable amount of time. For example, we can filter out stops with headways above 100 minutes. 
