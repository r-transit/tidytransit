...
```{r}
gtfs <- set_servicepatterns(gtfs)
```

`servicepatterns` is a list containing all date patterns and `service_servicepattern` links 
each `servicepattern_id` to a `service_id` (and by extension trips). We can compare the number 
of servicepatterns to the number of service_ids.
...
The feed uses 70 service_ids but there are actually only 7 different date patterns used by 
trips. Other feeds might not have such low numbers, for example the Swiss GTFS feed uses 
around 15'600 service_ids which all identify unique date patterns.
...
The plot shows that pattern `p_4df51d2` runs on every sunday from July until October without 
exceptions. `p_7b52f85` also runs on sundays but it also covers a monday (September 3rd). 
Similarly, the date pattern `p_2f5fffa` runs every saturday. `p_aadaafd` covers weekdays 
(mondays through friday), `p_279652d` seems to do the same through November with some exceptions. 
...
```{r echo=FALSE}
knitr::kable(servicepattern_names)
```
