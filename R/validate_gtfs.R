...
make_var_val <- function() {
  ...
  all_df <- bind_rows(agency_df, stops_df, routes_df, trips_df, stop_times_df, calendar_df, calendar_dates_df, 
...
validate_vars_provided <- function(val_files, feed) {
  ...
  if (nrow(extra_files_df) > 0) {
    ...
    for (i in extra_files_df) {
      ...
      extra_vars_df <- bind_rows(extra_vars_df, temp_vars_df)
      
    }
  
    spec_vars_df <- bind_rows(spec_vars_df, extra_vars_df)
  ...
  for (j in val_files_df) {
    ...
    val_vars_df <- bind_rows(val_vars_df, temp_vars_df)
  ...
  all_df <- bind_rows(sub_all_df, case1_df)
