...
_By default, `filter_feedlist()` only checks to make sure each links starts with *http://*. To check the link is actually working, use option `test_url = TRUE`. But beware, this can take a while!_
...
As an example, let's extract the data and problems data frames for the 38th url of `feedlist_df`.

```{r problems, warning=FALSE, message=FALSE, echo=TRUE, eval=TRUE, results='hide'}
url <- feedlist_df %>% slice(38) %>% select(url_d)
...
The `gtfsr` has mapping functions---wrapped around functions from the `sp` and `leaflet` packages---designed to help users quickly map GTFS feed spatial data. These functions input `gtfs` objects and then map the desired datum or data.
...
First, let's extract a `gtfs` object for Duke University's transit feed.
...
Finally, we can get visualize all of the routes that make up Duke University's Transit system using `map_gtfs_agency_routes`. Duke University Transit system is made of only one agency (agency `r duke_gtfs_obj$agency_df$agency_id`).
