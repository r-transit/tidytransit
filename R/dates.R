...
#' 
#' @importFrom dplyr filter select inner_join full_join anti_join join_by as_tibble tibble
...
set_dates_services <- function(gtfs_obj) {
  ...
  if(has_calendar) {
    ...
    if(has_calendar_dates) {
      ...
      if(nrow(additions) > 0) {
        date_service_df <- full_join(date_service_df, 
                                            additions, 
      ...
      if(nrow(exceptions) > 0) {
        date_service_df <- anti_join(date_service_df, 
                                            exceptions, 
                                            by=c("date", "service_id"))
      }
    }
