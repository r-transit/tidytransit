...
# library(gtfsr)
devtools::load_all("/Users/danton/GitHub/gtfsr-opensci")
library(magrittr)
library(dplyr)
...
Once we have the urls for the feeds of interest, we can download and extract all the GTFS data into a list of `gtfs` objects using the `import_gtfs` data. To save time, we will only download 2 feeds (`slice(8:9)`). The output is suppressed because of how verbose it is.
...
gtfs_objs1 <- url %>% import_gtfs
```

If you look at the output when creating the `gtfs_objs1` object, you should see output that looks like this...
...
df <- gtfs_objs1[[1]]$calendar_df # extract `calendar_df` from the 1st list element (there's only one)
df
attr(df, 'problems')
...
GTFS feeds contain numerous *required* and *optional* files. And within each of these files, there are also *required* and *optional* fields (variables). (For more detailed information, please see Google's [GTFS Feed Specification Reference](https://developers.google.com/transit/gtfs/reference).)
...
The `gtfsr` package can quickly check the file and field structure of a GTFS feed and inform you if all required files and fields have been found, are non-empty, and additional informationa about optional files and fields. The function is called `validate_gtfs_structure()`. It inputs an object of class `gtfs` (the output of functions `import_gtfs()` or `read_gtfs()`) and, by default, attaches a `validate` attribute (i.e. `attr(gtfs_obj, 'validate')`) to the `gtfs` object. There is also the option to return only the validation information (`return_gtfs_obj = FALSE`).

Let's take a look at an example, using transit feed data from agencies in Durham, NC, USA.


```{r validate, warning=TRUE, message=TRUE, echo=TRUE, eval=TRUE}
nc <- feedlist_df %>%
...
gtfs_objs2 <- durham_urls %>% import_gtfs # extract a gtfs_objs
sapply(gtfs_objs2, class) # verify that each object of is a `gtfs` object

## validate file and field structures
gtfs_objs_w_validate <- lapply(gtfs_objs2, validate_gtfs_structure) # attach `validate` data as attribute
validate_list_attr <- lapply(gtfs_objs_w_validate, attr, which = 'validate') # extract `validate` attribute data

validate_list_direct <- lapply(gtfs_objs2, validate_gtfs_structure, return_gtfs_obj = FALSE) # extract validation data directly

## both methods work. option `return_gtfs_obj = FALSE` allows a more direct way to get validation data
...
# OR
#validate_list_direct[[3]]$problem_opt_files
```
...
## 4. Mapping stops using `gtfsr` and the `leaflet` package

Let's try mapping stops for a bus route popular with Duke University students---the *C1 East-West Loop*. This route bounces between Duke's East and West campus. The *C1* idles at one of the busiest stop at Duke---the "West Campus Chapel" stop.

This bus stop is located in front of Duke University's iconic gothic Chapel and is the starting point for most folks visiting Duke's campus.

First, let's extract a `gtfs` object for Duke University.
...
duke_gtfs_obj <- nc %>%
    ...
    select(url_d) %>% # get duke univeristy
    import_gtfs
```

Next, we need to find the ID of the "West Campus Chapel" stop. We can search the required field `stop_name` for something that matches with a clever combination of `dplyr::slice` plus `which` and ``grepl`.

```{r duke-match, include=TRUE}
# look for west chapel stop
west_chapel_id <- duke_gtfs_obj[['stops_df']] %>%
    slice(which(grepl('west campus chapel', stop_name, ignore.case=TRUE))) %>% # slice rows where we match on name
...
```{r duke-map, include=TRUE}
map_gtfs_stop(gtfs_obj = duke_gtfs_obj, stop_id = west_chapel_id)
```
