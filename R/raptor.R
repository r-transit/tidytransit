...
#' @param separate_starts If `FALSE` (default) initial transfers among stops in
#'   `stop_ids` are returned. If `TRUE` each stop_id is calculated independently. This leads 
#'   leads to slightly faster calculation time and can be used when the resulting times 
#'   between `from` and `to_stop_ids` are aggregated later (for example by stop_name).
#' 
