...
This output was suppressed in the last section to save space given how verbose it is. But the highlighted `NOTE` explains that *if one observes an error or warning during the import process*, one can use the `problems = TRUE` option (`FALSE` by default), which allows you to extract a problems dataframe.

As an example, let's extract the data and problems dataframes for the 44th url of `feedlist_df`.
...
These error are mostly due to the fact that empty rows were added to the end of the text file. But assume we don't know that. It is therefore prudent to extract the problems dataframe to keep a record of potential problems. Let's compare the `calendar_df` dataframe from each list.
