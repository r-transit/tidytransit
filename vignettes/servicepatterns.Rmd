...
suggest_servicepattern_names = function(servicepattern, calendar) {
  ...
  # check for holidays
  holiday_summary = calendar_excerpt %>% group_by(weekday) %>% 
    summarise(
      count = dplyr::n(),
      holiday_count = length(holiday[!is.na(holiday)]),
      holiday_names = ifelse(any(!is.na(holiday)), paste(holiday, collapse=", "), "")
    ) %>% arrange(desc(count))
  
  # Multiple weekdays with holidays
  if(sum(holiday_summary$holiday_count) > 0) {
    pattern_name = weekday_plural(holiday_summary$weekday[1])
    holiday_names = holiday_summary$holiday_names[holiday_summary$holiday_names != ""]
    pattern_name <- paste0(pattern_name, " and ", paste(holiday_names, collapse = ", "))
